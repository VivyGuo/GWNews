...
@implementation GWActivityIndicator
...
- (void)setMode:(GWActivityMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    //，
    if (mode == GWActivityModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(GWRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == GWActivityModeDeterminate) {
        self.indicator = [[[GWRoundProgressView alloc] init] autorelease];
    }
    else if (mode == GWActivityModeCustomView && self.customView != nil){
...
+ (GWActivityIndicator *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	GWActivityIndicator *hud = [[GWActivityIndicator alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[GWActivityIndicator class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		GWActivityIndicator *HUD = (GWActivityIndicator *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"GWActivityViewIsNillException" 
					format:@"The view used in the GWActivity initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = GWActivityAnimationFade;
        self.mode = GWActivityModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == GWActivityAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == GWActivityAnimationZoom) {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetGrayFillColor(context, 0.2f, self.opacity);
...
@implementation GWRoundProgressView
