...
#import "GWFileUtility.h"
#import <sys/stat.h>
#import <dirent.h>
 
@implementation GWFileUtility
...
+ (BOOL)renameFile:(NSString *)filePath toFile:(NSString *)toPath
{
    NSFileManager *fm = [NSFileManager defaultManager];
    NSError *error = nil;
    if ([GWFileUtility isFileExist:toPath]) {
           ...
           isReplace:(BOOL)isReplace
{
    ...
    if ([GWFileUtility isFileExist:toPath] && isReplace) {
        [GWFileUtility deleteFile:toPath];
    }
                   ...
                   isReplace:(BOOL)isReplace
{
    ...
    for (NSString *path in contents) {
        ...
        if ([GWFileUtility isFileExist:toFilePath] && isReplace) {
            [GWFileUtility deleteFile:toFilePath];
        }
...
+ (double)calculteFileSzie:(NSString *)filePath
{
    ...
    if (dirContents == nil) {//文件夹为空时,dirContents不为nil但数目count为0,path为文件时,dirContents=nil
        ...
        for (NSString *dirName in dirContents) {
            fSize +=  [GWFileUtility calculteFileSzie:[filePath stringByAppendingPathComponent:dirName]] ;
        }
    }
    ...
    if (dir != NULL) { 
        ...
        while ((child = readdir(dir))!=NULL) {
            ...
            if (child->d_type == DT_DIR) { 
                folderSize += [GWFileUtility calculteFileSizeAtPath:childPath]; 
                // 把目录本身所占的空间也加上
...
+ (double)calculteFileSzieEx:(NSString *)filePath
{
    ...
    return [GWFileUtility calculteFileSizeAtPath:[filePath cStringUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (void)deleteFiles:(NSArray *)fileNames inPath:(NSString *)path
{
    ...
    if (dirContents == nil) {//文件夹为空时,dirContents不为nil但数目count为0,path为文件时,dirContents=nil
        if ([fileNames containsObject:[path lastPathComponent]]) {
            [GWFileUtility deleteFile:path];
        }
    }
    else {
        for (NSString *dirName in dirContents) {
            if ([fileNames containsObject:dirName]) {
                [GWFileUtility deleteFile:[path stringByAppendingPathComponent:dirName]];
            }
            else {
                [GWFileUtility deleteFiles:fileNames inPath: [path stringByAppendingPathComponent:dirName]];
            }
        }
    }
}
